...
public class ScanAdapter extends RecyclerAdapter<BleRssiDevice> {
    ...
    public void setConnectListener(View.OnClickListener listener){
...
//        BleDevice device = rssiDevice.getDevice();
        ...
        if (mConnectListener != null){
            hepler.getView(R.id.tv_connect).setTag(device);
            hepler.getView(R.id.tv_connect).setOnClickListener(mConnectListener);
        }
...
//                    ble.stopScan();
//                }
//                Log.e("xlh","device:"+device.getBleAddress());
        ...
        hepler.getConvertView().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(opened == hepler.getAdapterPosition()) {
                    //当点击的item已经被展开了, 就关闭. When the clicked item has been expanded, it is closed.
                    opened = -1;
